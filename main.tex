...
% Evita destinos duplicados de hipervínculos en figuras/tablas.
\makeatletter
...
\title{\texorpdfstring{MÁSTER EN CIBERSEGURIDAD \\ \vspace{0.5cm} \large MÓDULO 11. TRABAJO FIN DE MÁSTER (TFM)}{MÁSTER EN CIBERSEGURIDAD - MÓDULO 11. TRABAJO FIN DE MÁSTER (TFM)}}
...
Como formulación operativa del estudio, se define la siguiente pregunta de investigación: \textit{¿en qué medida la emisión dinámica de credenciales con revocación verificable reduce la ventana de exposición frente al uso de secretos estáticos en un pipeline de \gls{cicd} \textit{on-premise}?}
...
Finalmente, desde el punto de vista de la ingeniería de la infraestructura, se postula que la orquestación de este ecosistema mediante paradigmas de Infraestructura como Código (\gls{iac}), al desvincular la capa de gestión de acceso de la topología de red subyacente, proporciona la abstracción necesaria para escalar el modelo operativo. El uso de manifiestos declarativos y automatización modular facilita la exportación de esta misma arquitectura desde un entorno on-premise virtualizado hacia un modelo de nube híbrida pública, manteniendo la cohesión y las políticas de seguridad inmutables sin requerir una reingeniería profunda de la lógica de acceso.

Para hacer esta hipótesis falsable en el laboratorio, se adoptan tres criterios de verificación: (1) que la credencial emitida sea temporal y quede acotada por \gls{ttl}; (2) que la revocación elimine el acceso y el rol efímero en el sistema objetivo; y (3) que una identidad sin política suficiente reciba denegación de acceso. El trabajo experimental se considera consistente con la hipótesis únicamente cuando estos tres criterios se observan de forma simultánea en ejecución real.
...
El núcleo del sistema reside en una bóveda criptográfica centralizada (como HashiCorp \gls{vault}), configurada para operar como la única fuente de verdad para el material sensible. En este TFM, la implementación experimental se ha realizado en modo de nodo único con almacenamiento \gls{raft}; por tanto, la alta disponibilidad multi-nodo queda fuera de alcance en este laboratorio. La arquitectura se segmenta en tres planos funcionales:
...
Para la validación empírica de la arquitectura propuesta, se ha desplegado un laboratorio \textit{on-premise} sobre un hipervisor tipo 1 (Proxmox VE 8.4.17). La infraestructura de hardware asignada consta de 48 GB de memoria RAM, un disco de estado sólido (SSD) de 4 TB para el almacenamiento operativo de alta velocidad gestionado mediante aprovisionamiento fino (\textit{thin provisioning}), y un disco duro mecánico (HDD) de 1 TB destinado a volcados de seguridad periódicos (\texttt{vzdump}).

En este TFM, el laboratorio no se utiliza como entorno de demostración visual, sino como instrumento experimental para contrastar la hipótesis bajo condiciones controladas y reproducibles. Por ello, cada resultado reportado se vincula a una prueba ejecutable, a un artefacto técnico y a un criterio explícito de aceptación o rechazo.
...
\begin{itemize}
    \item \textbf{\gls{lxc} 1 (Gitea v1.22.6 + Act Runner):} Plataforma ligera para el control de versiones y la ejecución de pipelines de \gls{cicd}, configurada para emitir tokens \gls{oidc}.
...
Para mantener el foco en la hipótesis y en la evidencia experimental, el resto de listados auxiliares (variables, inventarios, playbooks de soporte y scripts operativos de empaquetado) se dejan referenciados en el repositorio, evitando su reproducción íntegra en el cuerpo principal de la memoria.
...
La validación de la arquitectura se basa en criterios funcionales y en evidencias observables en el laboratorio. En concreto, se considera que el diseño funciona si se cumple lo siguiente: (1) la emisión de credenciales para acceso a base de datos es efímera y conduce a la revocación del rol mediante la operación \texttt{DROP ROLE}; (2) la trazabilidad de accesos se materializa en eventos de auditoría en formato JSON generados por \gls{vault}; y (3) la configuración del entorno y de las políticas se puede reproducir a partir de manifiestos declarativos versionados con Terraform \parencite{vault_database_secrets_engine,vault_audit_devices,terraform_documentation}. En esta instalación, el endpoint de Proxmox (\texttt{https://proxmox:8006/}) se resuelve por DNS de Tailscale, lo que condiciona la conectividad de administración remota al plano de red superpuesto.

Para mantener consistencia entre hipótesis, método y veredicto, la evaluación se ancla en tres aserciones de contraste:
\begin{enumerate}
    \item \textbf{Aserción A (temporalidad):} la credencial dinámica debe emitirse con \gls{ttl} acotado y observable.
    \item \textbf{Aserción B (revocación):} tras revocar la \textit{lease}, la credencial debe fallar en autenticación y el rol efímero no debe existir en \texttt{pg\_roles}.
    \item \textbf{Aserción C (control de abuso):} una identidad sin política suficiente debe recibir denegación al solicitar \texttt{database/creds/readonly-role}.
\end{enumerate}
La hipótesis se considera respaldada únicamente cuando A, B y C se verifican de forma simultánea en ejecución real.

Las fuentes de evidencia utilizadas incluyen la topología del laboratorio (Figura \ref{fig:topologia}), la automatización del host (Listado \ref{lst:setup}), la lógica de aprovisionamiento declarativo (Listado \ref{lst:terraform-main}) y la orquestación de configuración por Ansible (Listado \ref{lst:ansible-site}). Como control de reproducibilidad del código de infraestructura, se incorpora además la validación sintáctica y semántica de manifiestos mediante \texttt{terraform validate}, junto con la separación explícita de parámetros sensibles de autenticación de la API de Proxmox respecto al código versionado.

Como verificación operativa complementaria, se ejecutaron pruebas no destructivas por \texttt{SSH} sobre el host Proxmox del laboratorio para confirmar identidad de ejecución y estado base de plataforma (\texttt{hostname}, \texttt{id}, \texttt{pve-manager}). Para reforzar el principio de mínimo privilegio, estas comprobaciones se realizaron con un usuario técnico dedicado (\texttt{tfm-test}) y uso explícito de \texttt{sudo -n} únicamente sobre comandos de observación del laboratorio. Adicionalmente, en la VM 504 se validó la rotación de clave \texttt{SSH} hacia una credencial temporal renovada, comprobando tanto el acceso efectivo al sistema como la persistencia de la clave en la configuración \textit{cloud-init} (\texttt{qm config 504}). Este paso permite acotar el riesgo previo a cualquier \textit{apply}, al validar que el contexto remoto y los permisos de operador son los esperados antes de introducir cambios de estado.

Desde la estación de operación (macOS), se añadió un control de endurecimiento sobre la identidad del operador: la clave privada \texttt{SSH} se gestiona mediante \textit{Keychain} y \texttt{ssh-agent}, con desbloqueo local protegido por biometría (Touch ID) en el flujo de acceso al llavero. Este enfoque evita incrustar claves o passphrases en scripts, reduce exposición de secretos en terminal y refuerza la trazabilidad del acceso administrativo sin alterar el principio de automatización reproducible del laboratorio.

\section{Resultados}

\subsection{Matriz de verificación operativa (evidencia directa)}
Con el objetivo de aportar trazabilidad verificable para la defensa, la Tabla \ref{tab:verificacion-operativa} resume las comprobaciones ejecutadas sobre el entorno real de Proxmox y su estado observado en el laboratorio.

\begin{table}[H]
\centering
\small
\begin{tabular}{>{\raggedright\arraybackslash}p{0.27\textwidth} >{\raggedright\arraybackslash}p{0.33\textwidth} >{\raggedright\arraybackslash}p{0.34\textwidth}}
\toprule
...
\addlinespace
Protección de identidad del operador (acceso remoto) &
Validación de acceso por \texttt{SSH} desde macOS con clave privada en \textit{Keychain} + \texttt{ssh-agent}, desbloqueo local por biometría (Touch ID) y ejecución con usuario \texttt{tfm-test} &
Operativa sin exposición de clave privada ni passphrase en scripts; autenticación de operador reforzada para administración del laboratorio con principio de mínimo privilegio. \\
\bottomrule
\end{tabular}
...
\begin{enumerate}
    \item Se solicitó una credencial dinámica (\path{database/creds/readonly-role}) y se obtuvo un usuario temporal único con \textit{lease} asociado.
    \item Con esa credencial se ejecutó una consulta satisfactoria en PostgreSQL (\path{pre_revoke_query=ok}).
    \item Se revocó la \textit{lease} en modo síncrono (\texttt{revoke\_rc=0} y confirmación de revocación exitosa).
    \item Tras la revocación, la misma credencial dejó de autenticar (resultado esperado: \texttt{failed\_expected}) y el rol temporal ya no existía en \texttt{pg\_roles} (\texttt{post\_revoke\_role\_exists=no}).
\end{enumerate}
...
La reproducibilidad del despliegue se valida verificando que el laboratorio on-premise se construye desde código versionado y declarativo. El aprovisionamiento del host se automatiza mediante el script del Listado \ref{lst:setup}, mientras que la configuración de políticas, motores y roles se define a través de manifiestos Terraform (Listado \ref{lst:terraform-main}). Sobre esta base, la configuración específica de cada nodo (Gitea Runner, Vault, PostgreSQL y K3s) se orquesta mediante playbooks Ansible idempotentes (Listado \ref{lst:ansible-site}), separados por rol técnico y reutilizables en nuevas ejecuciones del laboratorio. Este enfoque reduce la deriva operativa entre ejecuciones y refuerza la hipótesis de una implementación inmutable y auditable en el \gls{sdlc} \parencite{terraform_documentation}.

En la validación experimental se observó:
\begin{enumerate}
    ...
    \item El clúster \gls{k3s} queda funcional en la VM 504, con nodo \texttt{Ready} validado mediante \texttt{kubectl get nodes -o wide} tras verificar acceso \texttt{SSH} con la clave rotada y persistencia en \texttt{cloud-init}.
\end{enumerate}

\subsection{Pruebas complementarias con métricas, casos negativos y criterio de hipótesis}
Para reforzar la validez experimental, se ejecutó una batería adicional automatizada sobre Proxmox (artefacto: \path{artifacts/validation/proxmox_validation_results.txt}) orientada explícitamente a validar hipótesis y no solo disponibilidad de servicios. La prueba se diseña con tres aserciones técnicas:
\begin{enumerate}
    \item \textbf{Reducción de ventana de exposición:} el \gls{ttl} de la credencial dinámica debe quedar acotado y, tras revocación, la credencial no debe reutilizarse.
    \item \textbf{Revocación efectiva:} además del fallo de autenticación post-revocación, el rol efímero debe desaparecer de \texttt{pg\_roles}.
    \item \textbf{Bloqueo de abuso por política:} un token sin permisos sobre la ruta de credenciales dinámicas debe ser denegado.
\end{enumerate}
A partir de estas aserciones, el propio script calcula un veredicto por hipótesis (\path{supported/not_supported}) y un veredicto global (\path{hypothesis_overall}).
\par
Los resultados observados fueron:
\begin{itemize}
    \item \textbf{Tiempo de emisión de credencial dinámica:} \path{issue_ms=107} ms.
    \item \textbf{Tiempo de revocación síncrona:} \path{revoke_ms=103} ms.
    \item \textbf{\gls{ttl} efectivo de lease:} \path{lease_ttl_seconds=60}.
    \item \textbf{Prueba positiva de uso:} credencial recién emitida válida en PostgreSQL (\path{pre_revoke_query=ok}).
    \item \textbf{Prueba negativa de revocación:} tras revocar, la credencial deja de operar (\path{post_revoke_query=failed_expected}) y el rol efímero se elimina del catálogo (\path{post_revoke_role_exists=no}).
    \item \textbf{Prueba negativa de autorización:} un token sin política de lectura a \path{database/creds/readonly-role} recibe denegación (\path{unauthorized_token_access=denied_expected}).
\end{itemize}
En esta ejecución, los indicadores derivados fueron consistentes con apoyo de hipótesis: \texttt{ttl\_within\_expected\_window=yes}, \texttt{revocation\_effective=yes}, \texttt{unauthorized\_access\_blocked=yes} y \texttt{hypothesis\_overall=supported}. Estos resultados incrementan el nivel de evidencia más allá de la validación funcional cualitativa, al demostrar comportamiento temporal y de control de acceso con criterios verificables en ejecución real.
...
Los resultados observados en el laboratorio respaldan la hipótesis principal: la sustitución de credenciales estáticas por emisión dinámica reduce el tiempo durante el cual una credencial podría reutilizarse tras una potencial exposición, mitigando \gls{secretsprawl}. La convergencia de las tres aserciones (A: \gls{ttl} acotado, B: revocación efectiva y eliminación de rol, C: denegación de acceso no autorizado) refuerza la coherencia entre la pregunta de investigación, la metodología aplicada y la evidencia experimental reportada. Además, la trazabilidad habilitada en \gls{vault} y la arquitectura orientada a prevención son coherentes con buenas prácticas de secret management y secret scanning \parencite{owasp_secrets_management_cheat_sheet,github_secret_scanning,vault_audit_devices}.

\subsection{Amenazas a la validez y limitaciones}
Como limitación, el estudio se basa en un entorno de laboratorio on-premise con componentes específicos (Proxmox, \gls{lxc}, \gls{k3s}, \gls{vault} y PostgreSQL). La validez externa queda condicionada por la replicabilidad de la configuración y por la calidad de las políticas implementadas. Aunque esta versión sí incorpora métricas puntuales de emisión y revocación de credenciales (ms) y comprobaciones negativas de autorización, no se incluye todavía una campaña estadística extensa (múltiples corridas, percentiles y dispersión) ni una batería sistemática de resiliencia ante fallos transitorios, por lo que la evaluación mantiene un foco principalmente funcional y de trazabilidad operacional.


\subsection{Backlog priorizado de continuación}
Para evitar ambigüedad en la continuidad del trabajo, se propone un backlog técnico con criterios de aceptación verificables:
\begin{enumerate}
    \item \textbf{P1 -- Campaña estadística de estabilidad:} ejecutar corridas repetidas del flujo (mínimo 30) y reportar mediana, p95 y dispersión de \texttt{issue\_ms} y \texttt{revoke\_ms}. \textit{Aceptación:} reporte con métricas agregadas y tasa de éxito de aserciones A/B/C.
    \item \textbf{P2 -- Resiliencia ante fallos controlados:} introducir fallos transitorios (caída temporal de Vault, error de red hacia PostgreSQL, token inválido \gls{oidc}) y medir recuperación. \textit{Aceptación:} cada escenario debe finalizar con veredicto esperado (fallo controlado o recuperación) y evidencia en logs.
    \item \textbf{P3 -- Correlación operativa en \gls{siem}:} exportar eventos de auditoría JSON de \gls{vault} a un backend de correlación para detección de anomalías. \textit{Aceptación:} regla de alerta funcional para accesos denegados repetidos y prueba de disparo documentada.
    \item \textbf{P4 -- Ampliación de superficie de secretos:} extender el patrón de emisión efímera a certificados o credenciales de servicios adicionales. \textit{Aceptación:} al menos un nuevo tipo de secreto con \gls{ttl}, revocación y prueba negativa.
\end{enumerate}
...
En el alcance de este TFM, dicha mitigación queda respaldada por evidencia reproducible sobre temporalidad, revocación efectiva y bloqueo de accesos no autorizados.
