...
Para reforzar la validez experimental, se ejecutó una batería adicional automatizada sobre Proxmox (artefacto: \texttt{artifacts/validation/proxmox\_validation\_results.txt}) orientada explícitamente a validar hipótesis y no solo disponibilidad de servicios. La prueba se diseña con tres aserciones técnicas:
\begin{enumerate}
    ...
    \item \textbf{Bloqueo de abuso por política:} un token sin permisos sobre \texttt{database/creds/readonly-role} debe ser denegado.
\end{enumerate}
A partir de estas aserciones, el propio script calcula un veredicto por hipótesis (\texttt{supported/not\_supported}) y un veredicto global (\texttt{hypothesis\_overall}).
\par
